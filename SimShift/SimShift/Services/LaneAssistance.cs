...
{
    /// <summary>
    ///     Experimental module using computer vision to steer vehicle within motorway lanes. This module is NOT functional at
    ///     all.
    ///     The module was aimed at ETS2 to maintain focus on the vehicle's mirrors, and adjusting the heading error and
    ///     lateral error of the white road side lines to desired values.
    ///     Some struggles:
    ...
    public class LaneAssistance : IControlChainObj
    {
        ...
        public bool ButtonActive => DateTime.Now > this.ButtonCooldownPeriod;
        ...
        public double GetAxis(JoyControls c, double val)
        {
            switch (c)
            {
                case JoyControls.Steering: return this.SteerAngle;
        ...
        public bool Requires(JoyControls c)
        {
            switch (c)
            {
                case JoyControls.Steering: return this.Active;
        ...
        public void TickControls()
        {
            ...
            if (Main.GetButtonIn(JoyControls.LaneAssistance) && this.ButtonActive)
            {
                this.Active = !this.Active;
                this.ButtonCooldownPeriod = DateTime.Now.Add(new TimeSpan(0, 0, 0, 1));
                this.LockedSteerAngle = Main.GetAxisIn(JoyControls.Steering);
                Debug.WriteLine("Setting lane assistance to: " + this.Active);
            }

            var currentSteerAngle = Main.GetAxisIn(JoyControls.Steering);

            // User overrides steering
            if (this.Active && Math.Abs(currentSteerAngle - this.LockedSteerAngle) > 0.05)
            {
                this.Active = false;
        ...
        public void TickTelemetry(IDataMiner data)
        {
            ...
            if (true)
            {
                ...
                for (int segI = 0; segI < this.NearbySegments.Count; segI++)
                {
                    var seg = this.NearbySegments[segI];
                    if (seg == null)
                    {
                        continue;
                    }
                    ...
                    foreach (var sol in seg.Solutions)
                    {
                        ...
                        {
                            this.NearbySegments[segI].GenerateHiRes(sol);
                        }
                        ...
                        {
                            dist = dst;
                            activeSegment = this.NearbySegments[segI];
                            activeSegmentOption = sol;
                        }
                    }
                }

                if (!this.NearbySegments.Any(k => k != null) || dist > 5)
                {
                    this.FindNewSegments(route, me);
                    if (firstTry)
                    {
                        firstTry = false;
                        goto rescanSegment;
                ...
                this.SteerAngle = 0.5f - lineDistanceError / gain; // - angleDistancError * 0.1f;
