...
namespace SimShift.Services
{
    public class VariableSpeedTransmission : IControlChainObj
    {
        ...
        {
            if (c == JoyControls.Brake) return val + variableBrake;
            if (c == JoyControls.Throttle)
            {
                ...
                switch (ShiftingPhase)
                {
                    ...
                    default: return variableThrottle;
                }
            }
        ...
        public bool Requires(JoyControls c)
        {
            switch (c)
            {
                ...
                case JoyControls.Throttle: return Enabled;
                ...
                case JoyControls.GearRange1: return Enabled;
            }
            return false;
        }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                for (int k = 0; k < maxgears; k++)
                                {
                                    ...
                                    if (rpm < Main.Drivetrain.StallRpm && k > 0) continue;
                                    allStalled = false;
                                    if (orpm > Main.Drivetrain.MaximumRpm) continue;
                                ...
                                {
                                    // Hysterisis
                                    if (Math.Abs(curPower - calcPower) > 25) Gear = calcEfficiencyGear + 1;
                                }
                                if (Efficiency)
                                {
                                ...
                                if (Gear > 0 && Gear != data.Telemetry.Gear) ShiftingPhase = ShiftPhase.OffThrottle;
                            }
                        }
                        break;
        ...
        private bool GetRangeButton(int bt)
        {
            ...
            {
                if (ShiftingPhase == ShiftPhase.EngageRange) return RangeSwitchTruthTable(bt, currentRange, newRange);
                else return false;
            }
            return false;
        }
        ...
        {
            if (bt == -1) return Gear == -1 && ShiftingPhase != ShiftPhase.EngageRange;
            else
            {
