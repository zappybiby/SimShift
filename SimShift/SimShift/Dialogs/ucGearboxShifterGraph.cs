...
namespace SimShift.Dialogs
{
    public partial class ucGearboxShifterGraph : UserControl
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            var linePens = new[] { new Pen(Color.Red, 1.0f), new Pen(Color.GreenYellow, 1.0f), new Pen(Color.DeepSkyBlue, 1.0f), new Pen(Color.Yellow, 1.0f), new Pen(Color.Pink, 1.0f), };
            var linePens2 = new[] { new Pen(Color.DarkRed, 1.0f), new Pen(Color.DarkGreen, 1.0f), new Pen(Color.DarkBlue, 1.0f), new Pen(Color.Orange, 1.0f), new Pen(Color.Purple, 1.0f), };
            ...
            var index = 0;
            foreach (var load in this.simulatedLoads)
            {
                double speed = 0.0f;
                int gear = 1;
                double time = 0.0;
                ...
                try
                {
                    ...
                    for (time = 0; time < maxTime; time += timeStep)
                    {
                        ...
                        rpm = speed * this.config.Drivetrain.GearRatios[gear - 1] * 3.6;
                        var fuel = this.config.Drivetrain.CalculateFuelConsumption(rpm, load);
                        var power = this.config.Drivetrain.CalculateTorqueP(rpm < 500 ? 500 : rpm, throttle * load / 100.0) - this.config.Air.CalculateTorque(speed);

                        if (shiftTimeout > time)
                        {
                            power = 0;
                        }
                        ...
                        var bestGear = this.config.Lookup(speed * 3.6, throttle * load / 100.0);
